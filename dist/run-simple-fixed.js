...
async function getPortfolio() {
    ...
    const portfolio = await calculator.updateFromTrades();
    return { totalValue: portfolio.totalValue };
}
async function executeTrade(fromToken, toToken, amountHuman, reason) {
    // Since trade execution endpoint is broken, we'll simulate it
    if (DRY_RUN) {
        console.log(`[DRY] Would execute: ${fromToken} -> ${toToken}, amount: ${amountHuman}, reason: ${reason}`);
        return { success: true, simulated: true };
    }
    else {
        console.log(`[LIVE] Trade execution endpoint is currently broken. Simulating trade...`);
        return { success: true, simulated: true };
    }
}
...
async function main() {
    ...
    console.log("🔧 Using Portfolio Calculator (trade history reconstruction)");
    console.log("⚠️  Trade execution is simulated due to API endpoint issues");
    const startTs = Date.now();
    const endTs = startTs + TEST_MINUTES * 60 * 1000;
    // Get initial portfolio
    const pf0 = await getPortfolio();
    const nav0 = Number(pf0?.totalValue || 0);
    console.log(`💰 Starting NAV: $${nav0.toFixed(2)}`);
            ...
            for (const sym of TOKENS) {
                ...
                if (Date.now() - st.lastDecisionAt >= DECISION_MIN * 60 * 1000 && st.emaFast && st.emaSlow) {
                    ...
                    if (Math.abs(drift) >= DRIFT_THRESHOLD && tradeNotional >= MIN_LOT_USD && Date.now() - st.lastTradeAt >= TRADE_COOLDOWN_SEC * 1000) {
                        ...
                        console.log(` [${DRY_RUN ? 'DRY' : 'LIVE'}] ${side} ${sym} qty=${qty} ok=${res?.success}`);
